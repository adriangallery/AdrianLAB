...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (tokenIdNum >= 500 && tokenIdNum <= 1099) {
      ...
      try {
        // Cargar imagen SVG desde samuraizero (con fallback a GitHub)
        const assetPath = `samuraizero/${cleanTokenId}.svg`;
        console.log(`[render] 🥷 Cargando imagen SamuraiZERO (con fallback GitHub): ${assetPath}`);
        
        const svgBuffer = await loadLabimagesAsset(assetPath);
        if (!svgBuffer) {
          throw new Error(`SamuraiZERO image not found: ${assetPath}`);
        }
    ...
    // Canvas intermedio para renderizar todas las capas excepto el BACKGROUND
    const contentCanvas = createCanvas(1000, 1000);
    const contentCtx = contentCanvas.getContext('2d');
    console.log('[render] Canvas de contenido (sin background) creado');
    ...
    if (skintraitPath) {
      ...
      if (skintraitImage) {
        contentCtx.drawImage(skintraitImage, 0, 0, 1000, 1000);
        ...
        if (baseImage) {
          contentCtx.drawImage(baseImage, 0, 0, 1000, 1000);
    ...
    // LÓGICA ESPECIAL: Si hay serum aplicado, usar el skin del serum
      ...
      if (appliedSerum === "GoldenAdrian") {
        ...
        if (serumSkinImage) {
          contentCtx.drawImage(serumSkinImage, 0, 0, 1000, 1000);
            ...
            if (evMutation === "GoldenAdrian") {
              if (evSuccess === false) {
                ...
                try {
                  ...
                  contentCtx.drawImage(failImage, 0, 0, 1000, 1000);
    ...
    // Si hay un trait de skin excepcional, usarlo en lugar del skin base
      ...
      if (skinImage) {
        contentCtx.drawImage(skinImage, 0, 0, 1000, 1000);
      ...
      if (useMannequin) {
        ...
        try {
          ...
          contentCtx.drawImage(mannequinImage, 0, 0, 1000, 1000);
          ...
          if (fallbackImage) {
            contentCtx.drawImage(fallbackImage, 0, 0, 1000, 1000);
      ...
      if (equippedTraits[category]) {
        ...
        if (category !== 'ADRIAN' && category !== 'ADRIANGF') {
          ...
          if (traitId >= 30000 && traitId <= 35000) {
            ...
            if (traitImage) {
              contentCtx.drawImage(traitImage, 0, 0, 1000, 1000);
    ...
    if (equippedTraits['GEAR'] === '48') {
      ...
      if (specialTraitImage) {
        contentCtx.drawImage(specialTraitImage, 0, 0, 1000, 1000);
