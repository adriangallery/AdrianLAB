...
export function createCachedContract(contract, contractName) {
  ...
  // En ethers.js v5, contract.functions contiene funciones con firmas completas
  // También necesitamos exponer las funciones directamente desde el contrato
  ...
  // También exponer funciones directas del contrato (sin firma completa)
  // Esto asegura que funciones como 'getDupInfo' estén disponibles
  for (const key of Object.keys(contract.interface.functions)) {
    const baseName = key.split('(')[0]; // Extraer nombre sin parámetros
    if (!cachedContract[baseName] && typeof contract[baseName] === 'function') {
      cachedContract[baseName] = (...args) => {
        return cachedContractCall(contract, contractName, baseName, ...args);
      };
    }
  }
