...
export async function getDupInfo(duplicatorModule, tokenId) {
  ...
  try {
    // Llamar al contrato
    const result = await duplicatorModule.getDupInfoFlat(tokenIdStr);
    ...
    console.error(`[duplicator-cache] ❌ Error obteniendo dupInfo para token ${tokenIdStr}:`, error.message);
    return null;
  }
}
