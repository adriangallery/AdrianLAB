...
export function generateRenderHash(renderParams) {
  const {
    ...
    // ZEROmovies (movieId + overdue state for cache invalidation)
    movieId = null,
    overdueState = '',
    ...
    // Bounce on-demand (GIF). Cuando bounce.enabled=true el render produce
    // un GIF, así que el cache key debe diferenciarlo de los PNG estáticos.
    bounce = null,
  } = renderParams;
  ...
  const normalizedData = {
    ...
    // ZEROmovies
    movieId: movieId !== null ? String(movieId) : '',
    overdueState: overdueState ? String(overdueState) : '',

    // Duplicación
    duplicated: duplicated ? '1' : '0',
    dupNumber: String(dupNumber || 0),

    // Bounce — string compacto con todos los params relevantes; vacío si off
    bounce: bounce && bounce.enabled
      ? [
          bounce.direction,
          bounce.distance,
          bounce.bounces,
  ...
  // Orden de capas especial (GEAR en la pared): solo se añade la clave cuando aplica, para que el hash
  // del resto de tokens no cambie y su caché en GitHub siga valiendo.
  const gearIndex = traitCategories.findIndex((c) => String(c).toUpperCase() === 'GEAR');
  if (gearIndex !== -1 && GEAR_BEHIND_BODY.has(parseInt(traitIds[gearIndex]))) {
    normalizedData.layout = 'gear-wall-1';
  }
...
/**
 * Genera un hash único para un GIF de floppy animado
 * @param {string|number} traitId - ID del trait
 * @param {Object} tokenData - Metadata del token (name, category, floppy, maxSupply)
 * @param {Array} animatedVariants - Array ordenado de variantes del trait animado
 * @param {number} width - Ancho del GIF
 * @param {number} height - Alto del GIF
 * @param {number} delay - Delay por frame en ms
 * @returns {string} - Hash único (SHA-256, primeros 16 caracteres)
 */
export function generateFloppyGifHash(traitId, tokenData, animatedVariants, width, height, delay) {
  // Normalizar valores a strings consistentes
  const normalizedData = {
    traitId: String(traitId),
    name: String(tokenData?.name || ''),
    category: String(tokenData?.category || ''),
    floppy: String(tokenData?.floppy || ''),
    maxSupply: String(tokenData?.maxSupply || ''),
    // Ordenar variantes para consistencia
    variants: Array.isArray(animatedVariants)
      ? [...animatedVariants].sort().map(v => String(v)).join(',')
      : '',
    width: String(width || 768),
    height: String(height || 1024),
    delay: String(delay || 500),
    renderVersion: 'v4-2'
  };
...
/**
 * Genera el nombre de archivo para un GIF de floppy
 * @param {string|number} traitId - ID del trait
 * @param {string} hash - Hash único del render
 * @returns {string} - Nombre del archivo (ej: "123_floppy_gif_a1b2c3d4e5f6g7h8.gif")
 */
export function getFloppyGifFilename(traitId, hash) {
  return `${traitId}_floppy_gif_${hash}.gif`;
}

/**
 * Genera un hash único para un render custom basado en tokenId y traits
 * @param {string|number} tokenId - ID del token
 * @param {Array<string|number>} traitIds - Array de trait IDs (ordenados)
 * @returns {string} - Hash único (SHA-256, primeros 16 caracteres)
 */
export function generateCustomRenderHash(tokenId, traitIds) {
  const normalizedData = {
    tokenId: String(tokenId),
    traits: Array.isArray(traitIds) 
      ? [...traitIds].sort().map(id => String(id)).join(',')
      : ''
  };
...
/**
 * Genera el nombre de archivo para un render custom
 * @param {string|number} tokenId - ID del token
 * @param {string} hash - Hash único del render
 * @param {boolean} isGif - Si es GIF o PNG
 * @returns {string} - Nombre del archivo
 */
export function getCustomRenderFilename(tokenId, hash, isGif = false) {
  const ext = isGif ? 'gif' : 'png';
  return `${tokenId}_custom_${hash}.${ext}`;
}
