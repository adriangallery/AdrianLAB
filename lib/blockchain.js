...
export async function getTokenTraits(tokenId) {
  try {
    ...
    if (tokenId == 1) {
      // Determine body type based on tokenId - for token 1 we'll use athletic body (ID 1)
      const bodyTypeId = 1;
      const bodyTypes = [
      ...
      // Categories and trait IDs for token 1
      // We ensure it has BACKGROUND (1 = blue), MOUTH (1 = smile), and BASE (1 = athletic)
