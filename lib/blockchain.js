...
export async function getTokenTraits(tokenId) {
  try {
    ...
    console.log('[getTokenTraits] Contrato inicializado en:', extensions.address);
    ...
    console.log('[getTokenTraits] Llamando a getCompleteTokenInfo...');
    const tokenInfo = await extensions.getCompleteTokenInfo(tokenId);
    console.log('[getTokenTraits] Respuesta del contrato:', JSON.stringify(tokenInfo, null, 2));

    // Verificar que tenemos datos válidos
    if (!tokenInfo || !tokenInfo.tokenId) {
      console.error('[getTokenTraits] Datos inválidos recibidos del contrato');
      throw new Error('Token not found or data unavailable');
    }

    // Procesar la información usando la lógica de capas
    console.log('[getTokenTraits] Procesando información del token...');
    const processedInfo = processTokenInfo(tokenInfo);
