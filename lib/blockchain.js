...
export async function getTokenTraits(tokenId) {
  try {
    ...
    if (tokenId == 1) {
      ...
      // Categories y traitIds para el Token 1 - solo con los componentes SVG nuevos
      // y un fondo como capa inferior (BACKGROUND/1.png = blue.png)
      const categories = ["BACKGROUND", "BASE", "EYES", "HEAD", "CLOTHING"];
      const traitIds = [
        1,   // BACKGROUND: blue (PNG)
        11,  // BASE: SVG Gen0 body
        4,   // EYES: Classic glasses (SVG)
