...
export async function getTokenTraits(tokenId) {
  try {
    ...
    console.log('Contract initialized:', extensions.address);

    // Obtener información completa del token
    const tokenInfo = await extensions.getCompleteTokenInfo(tokenId);
    console.log('Raw token info:', JSON.stringify(tokenInfo, null, 2));

    // Verificar que tenemos datos válidos
    if (!tokenInfo || !tokenInfo.tokenId) {
      console.error('Invalid token data received from contract');
    ...
    console.log('Processed token info:', JSON.stringify(processedInfo, null, 2));
