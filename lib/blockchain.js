...
export async function getRawTokenMetadata(tokenId) {
  try {
    ...
    const { extensions, core } = await getContracts();
    console.log('[getRawTokenMetadata] Contratos inicializados:', {
      extensions: extensions.address,
      core: core.address
    });
    ...
    // Obtener datos completos del token para depuración
    const completeTokenData = await core.getCompleteTokenData(tokenId);
    console.log('[getRawTokenMetadata] Datos completos del token:', JSON.stringify(completeTokenData, null, 2));
