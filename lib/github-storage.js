...
export async function uploadFileToGitHub(tokenId, imageBuffer, renderType = 'normal') {
  try {
    ...
    const response = await serializedWrite(() => octokit.repos.createOrUpdateFileContents(uploadParams));
...
export async function deleteFileFromGitHub(filePath, reason = 'Invalidación manual') {
  try {
    ...
    const response = await serializedWrite(() => octokit.repos.deleteFile(deleteParams));
