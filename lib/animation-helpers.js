...
export function calculateBounceWithDelay(frameIndex, totalFrames, direction, distance, bounces, delayFrames = 0) {
  // Aplicar delay: frame efectivo = frameIndex - delayFrames
  const effectiveFrame = Math.max(0, frameIndex - delayFrames);
  const progress = totalFrames > 1 ? effectiveFrame / (totalFrames - 1) : 0;
