 ...
 *
 *  - BACKGROUND, ADRIAN, BODY, FACE, SKIN, SKINTRAIT  → 0  (cuerpo, sin lag)
 *  - EYES, MOUTH, NOSE, BEARD, HAIR                  → 1  (rasgos, lag corto)
 *  - HAT, HEAD, GLASSES, EAR, NECK, GEAR, SWAG, etc. → 2  (accesorios, lag medio)
 *  - TOP, OUTLINE                                    → 3  (capa cumbre, lag mayor)
 */
export function getCategoryDelayMultiplier(category) {
  ...
  if (['EYES', 'MOUTH', 'NOSE', 'BEARD', 'HAIR'].includes(c)) return 1;
  if (['HAT', 'HEAD', 'GLASSES', 'EAR', 'NECK', 'GEAR', 'SWAG', 'RANDOMSHIT', 'PAGERS', 'FLOPPY DISCS'].includes(c)) return 2;
  if (['TOP', 'OUTLINE'].includes(c)) return 3;
  return 1;
}
