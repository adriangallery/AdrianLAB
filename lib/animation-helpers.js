...
/**
 * Bounce con squash & stretch — pensado para bounce-on-demand sobre PFP completo.
 * Translate + scale no uniforme con anclaje en pies (anchorY ~0.92 del canvas).
 *
 * Curva: la altura se modela como |sin(progress*PI*bounces)| (idéntica a calculateBounceWithDelay)
 *  - bounceProgress ≈ 0  → impacto (cerca del suelo)  → squash (scaleY < 1, scaleX > 1)
 *  - bounceProgress ≈ 1  → cumbre del bote            → stretch (scaleY > 1, scaleX < 1)
 *
 * direction:
 *  - 'y'    → bote vertical (negativo en canvas = hacia arriba)
 *  - 'x'    → bote horizontal (sin squash gravitacional, solo translate)
 *  - 'both' → diagonal (mezcla)
 *
 * @param {number} frameIndex
 * @param {number} totalFrames
 * @param {Object} opts { direction, distance, bounces, delayFrames, squash, stretch }
 * @returns {Object} { x, y, scaleX, scaleY, rotation }
 */
export function calculateBounceSquash(frameIndex, totalFrames, opts = {}) {
  const {
    direction = 'y',
    distance = 50,
    bounces = 3,
  ...
  const impactWeight = Math.pow(1 - bounceProgress, 3);
  const peakWeight = Math.pow(bounceProgress, 2);

  const scaleY = 1 - squash * impactWeight + stretch * peakWeight;
  const scaleX = 1 + squash * 0.7 * impactWeight - stretch * 0.5 * peakWeight;
...
/**
 * Mapea categoría → multiplicador de delay (en frames) para bounce escalonado.
 * El multiplicador se aplica sobre `bounceConfig.delay` del usuario.
 *
 * Filosofía: el cuerpo y los rasgos faciales rebotan juntos (es UNA persona,
 * no un pelele articulado). Solo accesorios y la capa TOP llevan un lag muy
 * sutil para sugerir inercia (sombrero, capa, peinado largo).
 *
 *  - BACKGROUND          → no rebota (lo gestiona el frame generator aparte)
 *  - BODY/SKIN/cara/pelo → 0  (sin lag — es el sujeto que rebota)
 *  - HAT/HEAD/GLASSES…   → 1  (lag muy sutil)
 *  - TOP/OUTLINE         → 1  (mismo lag, no más)
 */
export function getCategoryDelayMultiplier(category) {
  const c = (category || '').toUpperCase();
  if (c === 'BACKGROUND') return 0;
  if (['ADRIAN', 'BODY', 'FACE', 'SKIN', 'SKINTRAIT'].includes(c)) return 0;
  if (['EYES', 'MOUTH', 'NOSE', 'BEARD', 'HAIR'].includes(c)) return 0;
  if (['HAT', 'HEAD', 'GLASSES', 'EAR', 'NECK', 'GEAR', 'SWAG', 'RANDOMSHIT', 'PAGERS', 'FLOPPY DISCS'].includes(c)) return 1;
  if (['TOP', 'OUTLINE'].includes(c)) return 1;
  return 0;
}

/**
 * @returns {boolean} true si la capa NO debe rebotar (background fijo).
 */
export function isStaticCategory(category) {
  return (category || '').toUpperCase() === 'BACKGROUND';
}
