...
export async function transformWithNanoBanana(pngBuffer, customPrompt = null) {
  try {
    ...
    // Configurar Google provider para usar Vercel AI Gateway
    ...
    // Configurar el modelo de Google con baseURL de AI Gateway si está disponible
    const apiKey = process.env.VERCEL_AI_GATEWAY_API_KEY || process.env.AI_GATEWAY_API_KEY;
    const baseURL = apiKey ? 'https://ai-gateway.vercel.sh/v1' : undefined;
    
    const googleModel = google('gemini-2.5-flash-image', {
      baseURL: baseURL,
      apiKey: apiKey,
    });
