...
/**
 * Obtiene el TTL específico para componentes SamuraiZERO
 * @param {string|number} componentId - ID del componente
 * @returns {number} - TTL en millisegundos
 */
export function getSamuraiComponentTTL(componentId) {
  const tokenIdNum = parseInt(componentId);
...
export function setCachedComponent(componentType, componentId, imageBuffer) {
  const key = `${componentType}_${componentId}`;
  const ttl = getSamuraiComponentTTL(componentId);
  ...
  console.log(`[component-cache] 💾 CACHE MISS: ${componentType}_${componentId} (${imageBuffer.length} bytes, TTL: ${Math.round(ttl/3600000)}h)`);
}
