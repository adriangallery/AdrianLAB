...
import { calculateBounceWithDelay, calculateBounceSquash, getCategoryDelayMultiplier } from './animation-helpers.js';
...
export function createBounceSquashFrameGenerator(config) {
  ...
  return async (frameIndex /* , totalFrames */) => {
    ...
    // 1) Capas estables (fondo + base + accesorios no animados) con su delay por categoría
...
export async function generateStandaloneAnimatedV4({ sourceGifBuffer, tokenIdNum, tokenData, totalMinted }) {
  ...
  console.log(`[gif-generator/v4-standalone] ${tokenIdNum}: ${totalFrames} frames, delay~${delayMs}ms, render w=${GIF_RENDER_W}`);
