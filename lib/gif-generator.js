...
export async function generateStandaloneAnimatedV4({ sourceGifBuffer, tokenIdNum, tokenData, totalMinted }) {
  ...
  // Render GIF frames at 75% of the static-PNG size (768×1050 instead of 1024×1400)
  // to fit Vercel's 1024 MB lambda memory cap — 12 frames at full size OOM'd.
  const GIF_RENDER_W = Math.round(V4_W * 0.75);
...
export async function generateFloppyGifV4(config) {
  ...
  const GIF_RENDER_W = Math.round(_V4W * 0.75);
