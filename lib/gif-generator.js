...
import { calculateBounceWithDelay, calculateBounceSquash, getCategoryDelayMultiplier, isStaticCategory } from './animation-helpers.js';
  ...
  for (let i = 0; i < layers.length; i++) {
    ...
    try {
      ...
      if (layer.transform) {
        ...
        // Punto de pivote dentro del canvas (en píxeles)
        ...
        // Dibujar la imagen colocando el ancla en el origen del transform
...
/**
 * Generador "bounce-only": un único PNG ya compuesto rebota como masa única.
 * Se usa cuando un PFP no tiene traits animados pero el usuario pide ?bounce=true.
 *
 * @param {Object} config
 * @param {Buffer} config.baseBuffer       — PNG completo del personaje compuesto
 * @param {Object} config.bounceConfig     — { direction, distance, bounces, frames, squash, stretch, anchorY }
 * @param {number} [config.width=1000]
 * @param {number} [config.height=1000]
 * @param {number} [config.delay=80]       — ms por frame
 * @returns {Function} customFrameGenerator(frameIndex, totalFrames)
 */
export function createBounceOnlyFrameGenerator(config) {
  const {
    baseBuffer,
    bounceConfig,
    width = DEFAULT_WIDTH,
  ...
  if (!baseBuffer) {
    throw new Error('createBounceOnlyFrameGenerator: baseBuffer es obligatorio');
  }
  ...
  return async (frameIndex /* , totalFrames */) => {
    const transform = calculateBounceSquash(frameIndex, totalFramesCfg, {
      direction,
      distance,
      bounces,
      delayFrames: 0,
    ...
    const frameLayers = [{
      pngBuffer: baseBuffer,
      transform: { ...transform, anchorX: 0.5, anchorY },
    }];

    const compositePng = await compositeLayers(frameLayers, width, height);
    return { pngBuffer: compositePng, delay };
  };
}

/**
 * Generador bounce multi-capa con squash & stretch + delay por categoría.
 * Usado por bounce-test (capas separadas) y por la rama animada de [tokenId]
 * cuando se quiera bounce escalonado por capa.
 *
 * @param {Object} config
 * @param {Array} config.layers           — [{ pngBuffer, category? }, ...] en orden Z
 * @param {Object} config.bounceConfig    — { direction, distance, bounces, frames, delay, squash, stretch, anchorY, perCategoryDelay }
 * @param {number} [config.width=1000]
 * @param {number} [config.height=1000]
 * @param {number} [config.delay=80]
 * @returns {Function} customFrameGenerator(frameIndex, totalFrames)
 */
export function createBounceSquashFrameGenerator(config) {
  const {
    layers = [],
    animatedTraits = [],
    bounceConfig,
  ...
  const {
    ...
    stretch = 0.06,
    anchorY = 0.92,
    perCategoryDelay = true,
  } = bounceConfig || {};

  // Pre-cargar variantes de traits animados (lazy, una sola vez)
  let variantsCachePromise = null;
  if (animatedTraits.length > 0) {
    variantsCachePromise = (async () => {
      const cache = new Map();
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://adrianlab.vercel.app';
      const loadJobs = [];
      for (const animTrait of animatedTraits) {
        ...
        for (const variantId of variants) {
          if (!cache.has(variantId)) {
            cache.set(variantId, null); // marker para evitar duplicados
            loadJobs.push(
              svgToPngBuffer(`${baseUrl}/labimages/${variantId}.svg`, width)
                .then(png => cache.set(variantId, png))
                .catch(err => {
                  console.warn(`[bounce-squash-gen] Error cargando variante ${variantId}: ${err.message}`);
  ...
  const transformFor = (frameIndex, layerDelay) => {
    const t = calculateBounceSquash(frameIndex, totalFramesCfg, {
      direction,
      distance,
      bounces,
      delayFrames: layerDelay,
      squash,
      stretch,
    });
    return { ...t, anchorX: 0.5, anchorY };
  };

  return async (frameIndex /* , totalFrames */) => {
    const variantCache = variantsCachePromise ? await variantsCachePromise : null;
    const frameLayers = [];

    // 1) Capas estables (fondo + base + accesorios no animados).
    //    BACKGROUND se dibuja SIN transform (queda fijo, independiente del bote).
    for (const layer of layers) {
      if (layer.isStatic || isStaticCategory(layer.category)) {
        frameLayers.push({ pngBuffer: layer.pngBuffer }); // sin transform → estática
        continue;
      }

      const layerDelay = perCategoryDelay
        ? getCategoryDelayMultiplier(layer.category) * delayUnit
        : (layer.delayOverride !== undefined ? layer.delayOverride : 0);

      frameLayers.push({
        pngBuffer: layer.pngBuffer,
        transform: transformFor(frameIndex, layerDelay),
      });
    }

    // 2) Variantes de traits animados — la variante depende del frame; el bounce
    //    delay viene de la categoría del trait
    if (variantCache) {
      for (const animTrait of animatedTraits) {
        const variants = animTrait.variants || [];
        if (variants.length === 0) continue;
        const variantId = variants[frameIndex % variants.length];
        const variantPng = variantCache.get(variantId);
        if (!variantPng) continue;

        const layerDelay = perCategoryDelay
          ? getCategoryDelayMultiplier(animTrait.category) * delayUnit
          : 0;
...
export async function generateStandaloneAnimatedV4({ sourceGifBuffer, tokenIdNum, tokenData, totalMinted }) {
  ...
  // Render GIF frames at 50% of the static-PNG size (512×700 instead of 1024×1400)
  // to fit Vercel's 1024 MB hobby lambda — 75% (768×1050) still OOM'd because
  // GifUtil.quantizeSorokin holds every frame in memory simultaneously.
  // Bitmap per frame: ~1.4 MB at 512×700 RGBA → 17 MB for 12 frames + overhead.
  const GIF_RENDER_W = Math.round(V4_W * 0.5);
  ...
  // Per-frame delays — using meta.delay[0] for everything makes a 1s static
  // intro frame stretch every frame to 1s, killing the animation. Real GIFs
  // typically have a long first frame + fast subsequent frames (e.g.
  // [1000,100,0,0,0,100,...] for floppy 10003).
  const delaysMs = Array.isArray(meta.delay) ? meta.delay : [];
  const fallbackDelayMs = 100;
  console.log(`[gif-generator/v4-standalone] ${tokenIdNum}: ${totalFrames} frames, delays=${JSON.stringify(delaysMs)}, render w=${GIF_RENDER_W}`);
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    const cardPng = new Resvg(svg, { fitTo: { mode: 'width', value: GIF_RENDER_W } })
      .render()
      .asPng();
    const png = PNG.sync.read(cardPng);
    const bitmap = new BitmapImage({ width: png.width, height: png.height, data: png.data });
    // Per-frame delay; clamp 0 (browser-default in source) to gifwrap min ~20ms.
    const frameDelayMs = (typeof delaysMs[i] === 'number' && delaysMs[i] > 0) ? delaysMs[i] : fallbackDelayMs;
...
export async function generateFloppyGifV4(config) {
  ...
  // Same memory-safety scaling as the standalone path: 768×1050 instead of full v4 size.
  const { V4_W: _V4W } = await import('./renderers/card-v4-renderer.js');
  const GIF_RENDER_W = Math.round(_V4W * 0.5);

  console.log(`[gif-generator/v4] Iniciando generateFloppyGifV4 para trait ${traitId} (render w=${GIF_RENDER_W})`);
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    const framePng = new Resvg(svg, { fitTo: { mode: 'width', value: GIF_RENDER_W } })
      .render()
      .asPng();
