...
export function resolveTokenName(tokenData, options = {}) {
  ...
  // 2.6. ZEROmovies2 (S2 — Return of the Pixel)
  if (tagInfo?.tag === 'ZEROmovies2' && tokenData._movieName) {
    return `ZEROmovies S2: ${tokenData._movieName} #${tokenId}`;
  }
...
export function resolveGeneration(tokenData) {
  ...
  if (tokenData.tagInfo?.tag === 'ZEROmovies2') {
    return TAG_CONFIGS.ZEROmovies2.metadataGenOverride;
  }
