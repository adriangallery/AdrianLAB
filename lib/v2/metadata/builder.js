...
function buildGumballMetadata(tokenData) {
  ...
  const version = Date.now();
  // GumballZERO composition only exists on the v2 render endpoint (v1 /api/render
  // has no gumball branch). Point the canonical image at v2.
  const imageUrl = `${BASE_URL}/api/v2/render/${tokenId}.png?v=${version}`;
  ...
  return {
    name: gumEntry.name || `GumballZERO #${tokenId}`,
