...
import { getMetadataFile, TOGGLE_MAP, BASE_URL, TAG_CONFIGS, CONTRACTS } from '../shared/constants.js';
...
export async function buildMetadata(tokenData, toggleEffects = {}) {
  ...
  // === GumballZERO (double-via: tag on-chain OR range match) ===
  if (isGumballToken(parseInt(tokenId), tagInfo?.tag)) {
    return buildGumballMetadata(tokenData);
  }
  ...
  // === ZEROmovies S1 (special metadata) ===
  if (tagInfo?.tag === 'ZEROmovies' && tokenData.movieId) {
    return buildZEROmoviesMetadata(tokenData);
  }

  // === ZEROmovies S2 — Return of the Pixel ===
  if (tagInfo?.tag === 'ZEROmovies2' && tokenData.movieId) {
    return buildZEROmovies2Metadata(tokenData);
  }
  ...
  // === V1 compatible: status field ===
  if (tokenData.tokenStatus) {
    metadata.status = tokenData.tokenStatus.hasProfile ? 'PATIENT' : 'NOT_PATIENT';
  } else {
    metadata.status = 'NOT_PATIENT';
  }

  // === V1 compatible: debug field ===
  metadata.debug = buildDebug(tokenData);
...
async function buildSamuraiMetadata(tokenData) {
  const { tokenId, honor = 0 } = tokenData;
  const samuraiIndex = tokenData._samuraiIndex;

  // Load SamuraiZERO metadata
  const samuraiData = loadJsonFile('samuraimetadata.json');
  if (!samuraiData || samuraiIndex === null || samuraiIndex === undefined || samuraiIndex < 0 || samuraiIndex >= 600) {
    return buildFallbackSamuraiMetadata(tokenId, honor);
  }

  const entry = samuraiData.collection?.[samuraiIndex];
  if (!entry) {
    return buildFallbackSamuraiMetadata(tokenId, honor);
  }

  // Extract base name and replace index with actual tokenId (V1 compatible)
  const nameMatch = entry.name?.match(/^(.+?)\s*#\d+$/);
  const baseName = nameMatch ? nameMatch[1] : (entry.name || 'SamuraiZERO').split('#')[0].trim();
  const finalName = `${baseName} #${tokenId}`;

  const version = Date.now();
  const imageUrl = `${BASE_URL}/api/render/${tokenId}.png?v=${version}`;

  // Honor (SamuraiDojo) — hidden when 0 to mirror UI HonorTag behaviour.
  ...
  const metadata = {
    name: finalName,
    description: entry.description || 'SamuraiZERO by HalfxTiger',
    image: imageUrl,
    external_url: entry.external_url || imageUrl,
    metadata_version: '2',
    attributes: [
  ...
  if (entry.masterminds) {
    metadata.masterminds = entry.masterminds;
  }

  return metadata;
}

function buildFallbackSamuraiMetadata(tokenId, honor = 0) {
  const honorAttribute = honor > 0
    ? [{ trait_type: 'Honor (名誉)', value: honor, display_type: 'number' }]
    : [];
  return {
...
function buildZEROmoviesMetadata(tokenData) {
  const { tokenId, movieId } = tokenData;
  const version = Date.now();

  // Load ZEROmovies metadata JSON
  const moviesData = loadJsonFile('zeromoviesmetadata.json');
  ...
  // Store movie name for name resolver
  tokenData._movieName = movieName;
  ...
  if (movieEntry?.franchise) {
    attributes.push({ trait_type: 'Franchise', value: movieEntry.franchise });
  }
  if (movieEntry?.genre) {
    attributes.push({ trait_type: 'Genre', value: movieEntry.genre });
  }
  if (movieEntry?.year) {
    attributes.push({ trait_type: 'Year', value: String(movieEntry.year) });
  }

  const metadata = {
    name: `ZEROmovies: ${movieName} #${tokenId}`,
    description,
    image: imageUrl,
    external_url: imageUrl,
    metadata_version: '2',
  ...
  // Animated version (GIF) if available
  if (movieEntry?.hasAnimation) {
    metadata.animation_url = `${BASE_URL}/labimages/zeromovies/animated/${movieId}.gif`;
  }

  return metadata;
}

// ===== ZEROmovies S2 METADATA =====

function buildZEROmovies2Metadata(tokenData) {
  const { tokenId, movieId, movieRental } = tokenData;
  const version = Date.now();

  const moviesData = loadJsonFile('zeromovies2metadata.json');
  const movieEntry = moviesData?.movies?.[String(movieId)];
  ...
  if (movieEntry?.hasAnimation) {
    attributes.push({ trait_type: 'Format', value: 'Animated' });
  }
  if (movieRental?.isOverdue) {
    attributes.push({ trait_type: 'Status', value: 'OVERDUE' });
    attributes.push({ trait_type: 'Days Overdue', value: String(movieRental.daysOverdue || 0) });
  }

  const metadata = {
    name: `ZEROmovies S2: ${movieName} #${tokenId}`,
    description,
    image: imageUrl,
    external_url: imageUrl,
    metadata_version: '2',
  ...
  // Animated version (GIF) — for S2, animated movies use the per-movie GIF
  // path defined in metadata. Major Dutch Schaefer (#42) is the only animated
  // movie at S2 launch.
  if (movieEntry?.hasAnimation) {
    const gifPath = movieEntry.animatedPath || `/labimages/zeromovies2/animated/${movieId}.gif`;
    metadata.animation_url = `${BASE_URL}${gifPath}`;
  }
...
/**
 * Build OpenSea-compatible metadata for a GumballZERO token.
 * Traits come from gums.json (curated off-chain), not from on-chain equipped traits.
 * Generation is always 0, skin is always Light.
 *
 * @param {Object} tokenData - From fetchAllTokenData (tagInfo.tag may or may not be 'GumballZERO')
 * @returns {Object} - OpenSea-compatible metadata JSON
 */
function buildGumballMetadata(tokenData) {
  const { tokenId } = tokenData;
  const version = Date.now();
  // GumballZERO composition is now handled by the v1 /api/render endpoint
  // (which has a full gumball branch). Point canonical image at v1.
  const imageUrl = `${BASE_URL}/api/render/${tokenId}.png?v=${version}`;

  // Use ordinal index set by the render endpoint (mirrors _samuraiIndex pattern).
  // Fallback to 0 when called from a path that hasn't resolved the index yet.
  const gumballIndex = tokenData._gumballIndex ?? 0;
  const gumEntry = getGumballTraits(gumballIndex);

  if (!gumEntry) {
    // Fallback: entry missing (race between mint and gums.json update, or index out of range)
    return {
      name: `GumballZERO #${tokenId}`,
  ...
  // Resolve trait names from traits.json for each traitId
  const traitsData = loadJsonFile('traits.json');
  const traitsArray = traitsData?.traits || [];
  const traitAttributes = [];

  for (const traitId of gumEntry.traitIds) {
    const entry = traitsArray.find(t => parseInt(t.tokenId) === parseInt(traitId));
    if (entry) {
      traitAttributes.push({ trait_type: entry.category, value: entry.name });
    }
  }
  ...
  return {
    // Name is dynamic by REAL tokenId (like the rest of the collection),
    // NOT gumEntry.name (which is the ordinal label "GumballZERO #1").
    name: `GumballZERO #${tokenId}`,
    description: gumEntry.description || 'BE REAL | BE ADRIAN | GumballZERO by HalfxTiger',
    image: imageUrl,
    external_url: gumEntry.external_url || 'https://adrianzero.com/',
    metadata_version: '2',
    masterminds: gumEntry.masterminds || ['Adrian | HalfxTiger'],
    attributes,
  };
}
...
function buildDebug(tokenData) {
  const {
    tokenId, originalGeneration, mutationLevel, canReplicate,
    replicationCount, lastReplication, hasBeenModified,
    skinId, skinName, categories, traitIds,
    serumHistory, tokenStatus, profileName,
  } = tokenData;

  // Resolve trait names for debug
  const traitNames = resolveTraitNames(categories, traitIds, tokenId);

  const usedSerum = (serumHistory && serumHistory.length > 0)
    ? (serumHistory[serumHistory.length - 1].success
      ? serumHistory[serumHistory.length - 1].mutation
      : 'FAILED')
    : 'No serums found';
  ...
  return {
    contracts: {
        ...
        functions: {
          getTokenData: {
            ...
            result: [
              ...
              String(lastReplication || 0),
              String(hasBeenModified),
            ],
          },
          getTokenSkin: {
            called: true,
            result: { skinId: String(skinId), skinName },
          },
        },
      },
      traitsExtension: {
        address: CONTRACTS.TRAITS_EXTENSION,
        functionCalled: 'getAllEquippedTraits',
        result: {
          categories,
          traitIds: traitIds.map(String),
          traitNames: traitNames.map((t, i) => ({
            category: categories[i],
            traitId: String(traitIds[i]),
            traitName: t.traitName,
          })),
        },
      },
      patientZero: {
        address: CONTRACTS.PATIENT_ZERO,
        functions: {
          getTokenStatus: {
            called: true,
            result: {
              status: tokenStatus?.hasProfile ? 'PATIENT' : 'NOT_PATIENT',
      ...
      serumModule: {
        address: CONTRACTS.SERUM_MODULE,
        functions: {
