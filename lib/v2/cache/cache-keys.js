...
// ===== TTLs (in seconds) =====
export const TTL = {
  RENDER_PNG:       604800,    // 7 days — rendered token image (traits change rarely)
  RENDER_GIF:       604800,    // 7 days — animated GIF
  METADATA_JSON:    86400,     // 24 hours — metadata JSON
  TRAIT_PNG:        604800,    // 7 days — pre-rendered trait PNG
  TOGGLES:          86400,     // 24 hours — toggle state
  TOKEN_DATA:       3600,      // 1 hour — on-chain data
...
/** Rendered token GIF (animated traits, bounce on-demand) */
export function renderGifKey(tokenId, hash) {
  return `v2:render-gif:${tokenId}:${hash}`;
}
