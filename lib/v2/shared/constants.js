...
export const CONTRACTS = {
  ...
  ZERO_DIAMOND: '0x542b2B96E9c944260722a86C2ee76166A8e3D0A0',
};

// ===== RPC CONFIGURATION =====
// NOTE: provider construction now lives in lib/contracts.js (rotating pool).
// Dead hardcoded Alchemy/Infura keys removed (they returned HTTP 403 "App is
// inactive"). This helper is kept env-only + base.org floor for any legacy caller.
export function getRpcUrls() {
  const alchemy = (k) => k && `https://base-mainnet.g.alchemy.com/v2/${k}`;
  return [
    ...(process.env.RPC_URLS || '').split(',').map((s) => s.trim()),
    alchemy(process.env.ALCHEMY_API_KEY_PRIMARY),
    alchemy(process.env.ALCHEMY_API_KEY),
    alchemy(process.env.ALCHEMY_API_KEY_FALLBACK),
    process.env.INFURA_PROJECT_ID ? `https://base-mainnet.infura.io/v3/${process.env.INFURA_PROJECT_ID}` : null,
...
export const SKIN_MAP = {
  ...
  // GumballZERO skin override — no on-chain skinId, forced by tag logic
  '5': { type: 'Light', name: 'Light' },
};

// ===== SPECIAL GEAR EXCEPTIONS =====
export const GEAR_BEFORE_SWAG = new Set([721, 726]);
export const GEAR_TOP_LAYER = new Set([48]);
// GEAR «colgados en la pared»: se pintan justo encima del BACKGROUND y por debajo de todo lo demás
// (cuerpo, skin y resto de traits). Siguen siendo GEAR. 1183 = Freed Soul (retrato enmarcado).
export const GEAR_BEHIND_BODY = new Set([1183]);
...
export const SPECIAL_TOKENS = {
  ...
  740: { image: '/labimages/specials/740.gif', name: 'ChuckZERO' },
  750: { image: '/labimages/specials/750.png', name: 'TAXreaper' },
  815: { image: '/labimages/specials/815.png', name: 'WakaZERO' },
};
...
export const TAG_CONFIGS = {
  ...
  ZEROmovies: {
    metadataGenOverride: 'ZEROmovies',
    imageBasePath: '/labimages/zeromovies/',
    metadataJsonPath: '/labmetadata/zeromoviesmetadata.json',
  },
  ZEROmovies2: {
    metadataGenOverride: 'ZEROmovies S2',
    imageBasePath: '/labimages/zeromovies2/',
    metadataJsonPath: '/labmetadata/zeromovies2metadata.json',
    animatedBasePath: '/labimages/zeromovies2/animated/',
  },
  GumballZERO: {
    metadataGenOverride: 'GumballZERO',
    metadataJsonPath: '/labmetadata/gums.json',
    fixedGeneration: 0,
    fixedSkinName: 'Light',
  },
};
...
// ===== GUMBALL PLACEHOLDER RANGE =====
// RECONCILIAR POST-PREMINT (F4): reemplazar con rango real asignado on-chain.
// Cambiar SOLO esta constante — getMetadataFile y tag-resolver la consumen.
export const GUMBALL_ID_RANGE = { start: 9001, end: 9100 };
...
export function getMetadataFile(tokenId) {
  ...
  if (n >= 20000 && n <= 20099) return { file: 'achievements.json', key: 'achievements' };
  if (n >= 30000 && n <= 35000) return { file: 'studio.json', key: null };
  if (n >= 100001 && n <= 101003) return { file: 'ogpunks.json', key: 'traits' };
  // GumballZERO (placeholder range — actualizar GUMBALL_ID_RANGE post-premint)
  if (n >= GUMBALL_ID_RANGE.start && n <= GUMBALL_ID_RANGE.end) return { file: 'gums.json', key: 'gums' };
