...
import { normalizeTraits, getLayerSequence, getWallLayers, getTraitLoadPath } from './layer-order.js';
...
export async function compositeToken(tokenData, options = {}) {
  ...
  // === Apply tag logic ===
  equippedTraits = applyTagLogic(equippedTraits, tagInfo, tokenData.tokenId, tokenData._gumballIndex ?? null);
  ...
  // === Step 1.5: GEAR hung on the wall (Freed Soul) — part of the background, below the body ===
  for (const { category, traitId } of getWallLayers(equippedTraits, animatedTraitIds)) {
    const { subdir, customPath } = getTraitLoadPath(category, traitId, tagInfo);
    const wallImg = await loadTraitImage(category, traitId, { subdir, customPath });
    if (wallImg) {
      ctx.drawImage(wallImg, 0, 0, 1000, 1000);
      if (bgCtx) bgCtx.drawImage(wallImg, 0, 0, 1000, 1000);
    }
  }
...
function applyTagLogic(traits, tagInfo, tokenId, gumballIndex = null) {
  if (isGumballToken(parseInt(tokenId), tagInfo?.tag)) {
    // Use ordinal index (0-based) resolved by getGumballIndex() in the render endpoint.
    // Fall back to 0 only when called from a cold path that hasn't set _gumballIndex.
    const idx = gumballIndex !== null ? gumballIndex : 0;
    const gumEntry = getGumballTraits(idx);
    if (gumEntry) {
