...
import { isTShitV2, resolveTShitUri } from '../rpc/tshit-resolver.js';
import { rasteriseStudioSvg } from './studio-rasterizer.js';

// Vercel doesn't serve public/ subdirectories (samuraizero/, ogpunks/) — use GitHub raw
const GITHUB_RAW = 'https://raw.githubusercontent.com/adriangallery/AdrianLAB/main/public';
...
export async function loadTraitImage(category, traitId, { subdir = null, customPath = null } = {}) {
  ...
  // Include subdir in KV key to avoid collisions (e.g. labimages/619.svg vs samuraizero/619.svg)
  const kvKey = subdir ? traitPngKey(subdir.toUpperCase(), id) : traitPngKey(category, id);
...
async function loadFromHTTP(traitId, { subdir, customPath }) {
  // Studio V2 (30014..35000): SVG URL lives on-chain via tshitGetDesignURI.
  // Resolve, fetch the SVG, and rasterise pixel-perfect (the embedded PNG
  // template is upscaled with canvas nearest-neighbor instead of Resvg
  // bilinear interpolation, so the t-shirt outline stays crisp at 1000×1000).
  let paths = buildHTTPPaths(traitId, { subdir, customPath });
  const numId = parseInt(String(traitId), 10);
  if (!subdir && !customPath && isTShitV2(numId)) {
    try {
      const onChainUri = await resolveTShitUri(numId);
      if (onChainUri) {
        const resp = await fetch(onChainUri, { signal: AbortSignal.timeout(5000) });
        if (resp.ok) {
          const svgBuf = Buffer.from(await resp.arrayBuffer());
          return await rasteriseStudioSvg(svgBuf);
        }
        console.warn(`[trait-loader] tshit ${numId} fetch ${resp.status}`);
      }
    } catch (err) {
      console.warn(`[trait-loader] tshit-resolver failed for ${numId}:`, err.message);
    }
    // Fall through to legacy paths if the on-chain branch fails.
  }
...
function buildFSPaths(traitId, { subdir, customPath }) {
  ...
  // When subdir is specified, ONLY search there (no generic fallback)
  // This prevents samuraizero/619.svg being overridden by labimages/619.svg
  if (subdir) {
    return [
      `labimages/${subdir}/${id}.svg`,
      `labimages/${subdir}/${id}.png`,
    ];
  }
...
function buildHTTPPaths(traitId, { subdir, customPath }) {
  ...
  // When subdir is specified, ONLY search there.
  // Vercel doesn't serve public/ subdirectories, so use GitHub raw.
  if (subdir) {
    return [
      `${GITHUB_RAW}/labimages/${subdir}/${id}.svg`,
      `${BASE_URL}/labimages/${subdir}/${id}.svg`,
    ];
  }
  ...
  if (numId >= 100001 && numId <= 101003) {
    urls.push(`${GITHUB_RAW}/labimages/ogpunks/${id}.svg`);
  }
