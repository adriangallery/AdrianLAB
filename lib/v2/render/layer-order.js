...
export function getLayerSequence(traits, animatedTraitIds = new Set()) {
  ...
  for (const category of TRAIT_ORDER) {
    ...
    // Skip GEAR hung on the wall (drawn right after the background, see getWallLayers)
    if (category === 'GEAR' && GEAR_BEHIND_BODY.has(parseInt(traitId))) continue;
...
/**
 * Layers drawn right after the background and before the body ("hung on the wall").
 * @param {Object} traits - Normalized equipped traits
 * @param {Set<string>} animatedTraitIds
 * @returns {Array<{category: string, traitId: string, phase: string}>}
 */
export function getWallLayers(traits, animatedTraitIds = new Set()) {
  const gear = traits['GEAR'];
  if (!gear || !GEAR_BEHIND_BODY.has(parseInt(gear)) || animatedTraitIds.has(gear)) return [];
  return [{ category: 'GEAR', traitId: gear, phase: 'wall' }];
}
