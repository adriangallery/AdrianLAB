...
import { encodeGetTokensByTag, encodeGumballGetAllMintedTokens, encodeGumballWasMintedHere } from '../rpc/contracts.js';
...
async function getGumballSortedList() {
  ...
  try {
    // GumballMintFacet owns its own tag registry (LibGumball slot). The shared
    // getTokensByTag selector belongs to SamuraiMintFacet (excluded from the
    // gumball cut) and returns [] for 'GumballZERO'. Use the gumball reader.
    const results = await multicall([
      encodeGumballGetAllMintedTokens(),
    ]);
...
/**
 * Self-contained GumballZERO resolution for callers without v2 tagInfo
 * (e.g. the v1 /api/metadata endpoint). Checks the on-chain gumball reader
 * directly (gumballWasMintedHere), then resolves ordinal index + gums.json entry.
 * @param {number|string} tokenId
 * @returns {Promise<{isGumball:boolean,index:number|null,entry:object|null}>}
 */
export async function resolveGumballForToken(tokenId) {
  try {
    const results = await multicall([encodeGumballWasMintedHere(Number(tokenId))]);
    const minted = results?.[0]?.success ? Boolean(results[0].data[0]) : false;
    if (!minted) return { isGumball: false, index: null, entry: null };
    const index = await getGumballIndex(Number(tokenId));
    const entry = (index !== null && index !== undefined) ? getGumballTraits(index) : null;
    return { isGumball: true, index, entry };
  } catch (err) {
    console.error('[tag-resolver] resolveGumballForToken failed:', err.message);
    return { isGumball: false, index: null, entry: null };
  }
}
