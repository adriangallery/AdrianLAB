...
export async function fetchAllTokenData(tokenId, { includeMetadata = false } = {}) {
  ...
  if (includeMetadata) {
    calls.push(
      encodeGetTokenStatus(id),                                     // 13: tokenStatus
      encodeGetTokenNameHistory(id),                                // 14: nameHistory
    );
  }
  ...
  // If ZEROmovies, fetch movieId + rental info (overdue detection)
  let movieId = null;
  ...
  if (includeMetadata) {
    tokenStatus = parseTokenStatus(results[13]);
    nameHistory = parseNameHistory(results[14]);
...
// ===== ZEROmovies rental config cache =====
// keepThreshold is set once at facet init and only changes via admin setRentalConfig.
// Cache it in-memory for ~1h to avoid 1 extra RPC call per token render.
let _rentalConfigCache = null;
let _rentalConfigFetchedAt = 0;
const RENTAL_CONFIG_TTL_MS = 60 * 60 * 1000; // 1h
 ...
 * Combines getRentalInfo(movieId) with cached getRentalConfig() (keepThreshold).
 *
...
async function fetchMovieRental(movieId) {
  try {
    ...
    const fetchConfig =
      !_rentalConfigCache || (Date.now() - _rentalConfigFetchedAt) > RENTAL_CONFIG_TTL_MS;
    if (fetchConfig) calls.push(encodeGetRentalConfig());
    ...
    if (fetchConfig && results[1] && results[1].success) {
      ...
      _rentalConfigCache = { keepThreshold: Number(c.keepThreshold || c[4]) };
      _rentalConfigFetchedAt = Date.now();
    }
    const keepThreshold = _rentalConfigCache?.keepThreshold ?? (30 * 24 * 60 * 60); // 30d default
    ...
    if (hasActiveRent && rentedAt > 0) {
      ...
      const expiresAt = rentedAt + keepThreshold;
      if (nowSec > expiresAt) {
        isOverdue = true;
        daysOverdue = Math.floor((nowSec - expiresAt) / 86400);
      }
    }
