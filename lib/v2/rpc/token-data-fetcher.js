...
export async function fetchAllTokenData(tokenId, { includeMetadata = false } = {}) {
  ...
  const calls = [
    ...
    encodeWasMintedHere(CONTRACTS.ZERO_DIAMOND, id),               // 9: wasMintedDiamond (ZEROmovies)
    encodeGetTokenTag(CONTRACTS.ZERO_DIAMOND, id),                 // 10: diamondTag (ZEROmovies)
  ];

  if (includeMetadata) {
    calls.push(
      encodeGetTokenStatus(id),                                     // 11: tokenStatus
      encodeGetTokenNameHistory(id),                                // 12: nameHistory
    );
  }
  ...
  const tagInfo = parseTagInfo(results[5], results[6], results[7], results[8], results[9], results[10]);

  // If ZEROmovies, fetch movieId via supplementary call
  let movieId = null;
  if (tagInfo.deployer === 'ZEROmovies') {
  ...
  if (includeMetadata) {
    tokenStatus = parseTokenStatus(results[11]);
    nameHistory = parseNameHistory(results[12]);
...
function parseTagInfo(subZeroMinted, subZeroTag, batchMinted, batchTag, diamondMinted, diamondTag) {
  // SubZERO check
  ...
  // ZEROmovies (Diamond) check
  if (diamondMinted && diamondMinted.success && diamondMinted.data[0] === true) {
    const tag = diamondTag && diamondTag.success ? (diamondTag.data[0] || '').trim() : 'ZEROmovies';
    return { isMinted: true, tag: tag || 'ZEROmovies', deployer: 'ZEROmovies' };
  }
