...
import {
  ...
  encodeGetHonor,
  encodeGumballWasMintedHere,
  encodeGumballGetTokenTag,
} from './contracts.js';
...
export async function fetchAllTokenData(tokenId, { includeMetadata = false } = {}) {
  ...
  const calls = [
    ...
    encodeGumballWasMintedHere(id),                                 // 15: wasMintedGumball (GumballMintFacet)
    encodeGumballGetTokenTag(id),                                   // 16: gumballTag
  ];

  if (includeMetadata) {
    calls.push(
      encodeGetTokenStatus(id),                                     // 17: tokenStatus
      encodeGetTokenNameHistory(id),                                // 18: nameHistory
      encodeGetHonor(id),                                           // 19: honor (SamuraiDojoFacet)
    );
  }
  ...
  const tagInfo = parseTagInfo(
    ...
    results[15], results[16], // Diamond / GumballMintFacet
  );
  ...
  if (includeMetadata) {
    tokenStatus = parseTokenStatus(results[17]);
    nameHistory = parseNameHistory(results[18]);
    honor = parseHonor(results[19]);
...
function parseTagInfo(
  ...
  gumballMinted, gumballTag
) {
  // 0. GumballMintFacet — checked FIRST because tokenIds are non-contiguous
  //    (minted into the global AdrianZERO counter alongside other collections),
  //    so range-based detection is NOT reliable in production. The tag
  //    'GumballZERO' is assigned at seed time by GumballMintFacet.
  if (gumballMinted && gumballMinted.success && gumballMinted.data[0] === true) {
    const tag = gumballTag && gumballTag.success ? (gumballTag.data[0] || '').trim() : '';
    return { isMinted: true, tag: tag || 'GumballZERO', deployer: 'Diamond:GumballMint' };
  }
