...
export function textToSVGElement(text, options = {}) {
  try {
    let pathData = textToPath(text, options);
    // Limpiar saltos de línea y comillas dobles
    pathData = String(pathData).replace(/\n/g, ' ').replace(/"/g, '&quot;');
    ...
    const textWidth = text.length * finalOptions.fontSize * 0.6;
    const textHeight = finalOptions.fontSize;
    
    // Ajustar la posición para centrar el texto
    const adjustedX = finalOptions.x - (textWidth / 2);
    const adjustedY = finalOptions.y + (textHeight / 2);
    ...
    // Crear grupo con transformación ajustada
    const group = `<g transform="translate(${adjustedX}, ${adjustedY})">${pathElement}</g>`;
    
    console.log(`[text-to-svg] Elemento SVG creado para "${text}" en (${adjustedX}, ${adjustedY})`);
    return group;
