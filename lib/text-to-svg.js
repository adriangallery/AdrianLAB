...
/**
 * Inicializa la instancia de TextToSVG con la fuente VT323 (retro)
 */
function initializeTextToSVG() {
  ...
  try {
    const fontPath = path.join(process.cwd(), 'public', 'fonts', 'retro', 'VT323-Regular.ttf');
    console.log(`[text-to-svg] Inicializando con fuente retro: ${fontPath}`);
    
    if (fs.existsSync(fontPath)) {
      textToSVGInstance = TextToSVG.loadSync(fontPath);
      console.log(`[text-to-svg] TextToSVG inicializado correctamente con VT323`);
      return textToSVGInstance;
    } else {
      console.error(`[text-to-svg] Fuente retro no encontrada: ${fontPath}`);
      throw new Error('Fuente VT323 no encontrada');
    }
  } catch (error) {
...
export function textToSVGElement(text, options = {}) {
  try {
    ...
    const defaultOptions = {
      ...
      stroke: null,
      strokeWidth: 0,
      anchor: 'center middle'  // Anclaje por defecto: centro exacto
    };
  ...
  try {
    const elements = lines.map(line => {
      return textToSVGElement(line.text, {
        ...
        anchor: line.anchor || 'center middle'
      });
    });
