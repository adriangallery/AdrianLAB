...
// Caché para getTokensByTag (TTL de 7 días - el orden de mint es inmutable)
const tokensByTagCache = new Map();
const TOKENS_BY_TAG_CACHE_TTL = 7 * 24 * 60 * 60 * 1000; // 7 días en millisegundos
...
export async function getTokensByTag(tag) {
  try {
    ...
    if (tokensByTagCache.has(cacheKey)) {
      ...
      if (expiry > Date.now()) {
        console.log(`[contracts] 🎯 CACHE HIT: getTokensByTag('${tag}')`);
        return value;
      }
    ...
    // Llamada al contrato
    console.log(`[contracts] 💾 CACHE MISS: getTokensByTag('${tag}')`);
    const subZeroDeployer = await getSubZeroDeployer();
    const tokenIds = await subZeroDeployer.getTokensByTag(tag);
