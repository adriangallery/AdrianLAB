...
// Import dinámico de SubZERO ABI para evitar problemas de build
let SubZEROABI = null;
async function loadSubZEROABI() {
  if (!SubZEROABI) {
    try {
      // Intentar usar require primero (funciona en Node.js)
      if (typeof require !== 'undefined') {
        try {
          SubZEROABI = require('./abis/SubZERO.json');
          return SubZEROABI;
        } catch (requireError) {
          // Si require falla, intentar con import dinámico
        }
      }
      // Fallback a import dinámico
      const module = await import('./abis/SubZERO.json');
      SubZEROABI = module.default || module;
    } catch (error) {
...
export async function getSubZeroDeployer() {
  try {
    const provider = await getProvider();
    const abi = await loadSubZEROABI();
