...
// Configuration — read from env at module load time.
// Hardcoded dead keys removed (those Alchemy apps are inactive as of May 2026).
// Priority order handled by _buildRpcUrl() below.
...
// Provider singleton — reused across all getXxx() calls within the same lambda invocation.
// This avoids redundant getNetwork() probes when multiple helper functions are called
// in a single metadata request.
let _cachedProvider = null;

// Build the RPC URL priority list at module load time (no probes, no timeouts).
// Priority: ALCHEMY_API_KEY_PRIMARY env → ALCHEMY_API_KEY env → ALCHEMY_API_KEY_FALLBACK env
//           → Infura env → Base public RPC.
// Hardcoded fallback keys have been removed: they were exhausted ("App is inactive").
function _buildRpcUrl() {
  // Use the first env-supplied key that is set
  if (ALCHEMY_RPC_URL_PRIMARY) {
    console.log('[contracts] Provider: using ALCHEMY_API_KEY_PRIMARY');
    return ALCHEMY_RPC_URL_PRIMARY;
  }
  // ALCHEMY_API_KEY env (may be overridden to a live key via Vercel env)
  if (process.env.ALCHEMY_API_KEY) {
    console.log('[contracts] Provider: using ALCHEMY_API_KEY');
    return `https://base-mainnet.g.alchemy.com/v2/${process.env.ALCHEMY_API_KEY}`;
  }
  if (process.env.ALCHEMY_API_KEY_FALLBACK) {
    console.log('[contracts] Provider: using ALCHEMY_API_KEY_FALLBACK');
    return `https://base-mainnet.g.alchemy.com/v2/${process.env.ALCHEMY_API_KEY_FALLBACK}`;
  }
  if (process.env.INFURA_PROJECT_ID) {
    console.log('[contracts] Provider: using INFURA_PROJECT_ID');
    return `https://base-mainnet.infura.io/v3/${process.env.INFURA_PROJECT_ID}`;
  }
  // Public Base RPC — no key needed, rate-limited but always available
  console.log('[contracts] Provider: using Base public RPC');
  return 'https://mainnet.base.org';
}

function getProvider() {
  if (!_cachedProvider) {
    const rpcUrl = _buildRpcUrl();
    _cachedProvider = new ethers.providers.JsonRpcProvider(rpcUrl, {
      name: 'Base Mainnet',
      chainId: 8453,
    });
  }
