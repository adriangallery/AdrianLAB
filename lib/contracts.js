...
// Alchemy Fallback (Segundo intento)
const ALCHEMY_API_KEY_FALLBACK = "fgoABFGfYfI7yIPOSW7_bHPiXLQuHPjU";
...
async function getProvider() {
  try {
    ...
    // Intentar primero con Alchemy principal
    ...
    console.warn('[contracts] Alchemy principal failed, trying Alchemy fallback...', error.message);
    try {
      // Segundo intento con Alchemy fallback
      ...
      return provider;
    } catch (alchemyFallbackError) {
      console.warn('[contracts] Alchemy fallback failed, trying Infura...', alchemyFallbackError.message);
      try {
        // Tercer intento con Infura
        ...
        console.error('[contracts] Infura también falló, usando Base RPC público...', infuraError.message);
