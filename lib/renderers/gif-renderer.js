...
export class GifRenderer {
  ...
  async loadTraitImage(traitId, category = null) {
    try {
      ...
      if (traitIdNum >= 30000 && traitIdNum <= 35000) {
        // Traits externos - usar URL externa directamente
        const imageUrl = `https://adrianzero.com/designs/${traitId}.svg`;
        const categoryLog = category ? ` (${category})` : '';
        console.log(`[GifRenderer] Cargando trait externo ${traitId}${categoryLog} desde: ${imageUrl}`);
        
        const response = await fetch(imageUrl);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        svgBuffer = Buffer.from(await response.arrayBuffer());
      } else if (traitIdNum >= 100001 && traitIdNum <= 101003) {
        // OGPunks traits - usar helper con fallback a GitHub
        const assetPath = `ogpunks/${traitId}.svg`;
        const categoryLog = category ? ` (${category})` : '';
        console.log(`[GifRenderer] Cargando trait OGPunk ${traitId}${categoryLog} desde (con fallback GitHub): ${assetPath}`);
        ...
        // Traits normales desde labimages (carpeta general, no por categoría) - usar helper con fallback a GitHub
        const assetPath = `${traitId}.svg`;
        const categoryLog = category ? ` (${category})` : '';
        console.log(`[GifRenderer] Cargando trait ${traitId}${categoryLog} desde labimages (con fallback GitHub): ${assetPath}`);
  ...
  async generateFrame(tokenId, traitId = null, category = null) {
    ...
    // GEAR «colgado en la pared» (Freed Soul): va detrás del cuerpo, justo encima del fondo
    const traitBehindBody = Boolean(traitId) && GEAR_BEHIND_BODY.has(parseInt(traitId));
    const traitImage = traitId ? await this.loadTraitImage(traitId, category) : null;
    if (traitImage && traitBehindBody) {
    ...
    // 3. Si hay trait, dibujarlo encima (salvo los de la pared, ya pintados detrás)
    if (traitId) {
      if (traitImage) {
        if (!traitBehindBody) ctx.drawImage(traitImage, 0, 0, 1000, 1000);
  ...
  async generateGif(config) {
    ...
    for (let i = 0; i < frameCanvases.length; i++) {
      ...
      try {
        ...
        // Crear BitmapImage convirtiendo el canvas a PNG y luego a bitmap (vía pngjs)
        // BitmapImage acepta un objeto { width, height, data: Buffer }
        const pngBuffer = canvas.toBuffer('image/png');
        const pngImage = PNG.sync.read(pngBuffer);
        
        // Crear BitmapImage pasando el objeto bitmap directamente al constructor
    ...
    console.log(`[GifRenderer] Cuantizando colores de ${gifFrames.length} frames a 256 colores...`);
    GifUtil.quantizeSorokin(gifFrames, 256);
    console.log(`[GifRenderer] Cuantización completada`);
    
    // Crear GIF usando GifCodec
