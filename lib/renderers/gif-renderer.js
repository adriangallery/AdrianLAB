...
export class GifRenderer {
  ...
  async generateGif(config) {
    ...
    console.log(`[GifRenderer] Convirtiendo ${frameBuffers.length} frames PNG a GifFrame...`);
    ...
    for (let i = 0; i < frameBuffers.length; i++) {
      ...
      console.log(`[GifRenderer] Procesando frame ${i + 1}/${frameBuffers.length}, tamaño: ${frameBuffer.length} bytes`);
      
      try {
        // Cargar PNG en Jimp
        // Usar importación dinámica si Jimp no está disponible
        let JimpInstance = Jimp;
        if (!JimpInstance || typeof JimpInstance.read !== 'function') {
          console.log(`[GifRenderer] Jimp no disponible estáticamente, usando importación dinámica...`);
