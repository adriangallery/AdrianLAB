 ...
 * @param {string} [args.traitB64Override] — pass a custom trait image (used for animated GIF frames + standalone items)
 * @param {boolean} [args.skipMannequin]   — true for standalone items (floppies, T-shits, OG covers)
 * @returns {string} svg
 */
export function buildV4CardSvg({ tokenIdNum, tokenData, totalMinted, traitB64Override, skipMannequin }) {
  const W = V4_W;
  const H = V4_H;
  const HS = V4_HS;
  const HX = V4_HX;
  ...
  const mannequinB64 = skipMannequin ? null : getMannequinB64();
