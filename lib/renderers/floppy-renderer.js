...
import { isTShitV2, resolveTShitUri } from '../v2/rpc/tshit-resolver.js';
...
export class FloppyRenderer {
  ...
  async generateSimpleSVG(tokenId, options = {}) {
    ...
    const simpleSvg = `
      <svg width="600" height="600" xmlns="http://www.w3.org/2000/svg">
        ...
        ${rarity.tag ? `<rect x="0" y="0" width="160" height="60" fill="${rarity.bg}"/>
  ...
  async #loadExternalTraitForFloppy(traitId) {
    try {
      // V2 (T-Shit Studio, Diamond mints): URL is stored on-chain in TShitMintFacet.
      // V1 (legacy 30000-30300): SVG was committed to the `adriangallery/adrianzero`
      // GitHub repo at `designs/{id}.svg` by the original V1 traitcreator at mint
      // time. The renderer reads them from raw.githubusercontent.com — the live
      // adrianzero.com domain now hosts the React SPA which 200s with index.html
      // for any `/designs/*` path, so falling back there returns invalid SVG.
      let imageUrl;
      if (isTShitV2(traitId)) {
        imageUrl = await resolveTShitUri(traitId);
        if (!imageUrl) {
          throw new Error(`No on-chain designURI for T-Shit V2 token ${traitId}`);
        }
        console.log(`[FloppyRenderer] 🌐 V2 designURI resolved: ${traitId} -> ${imageUrl}`);
      } else {
        imageUrl = `https://raw.githubusercontent.com/adriangallery/adrianzero/main/designs/${traitId}.svg`;
        console.log(`[FloppyRenderer] 🌐 V1 legacy URL (GitHub raw): ${traitId} -> ${imageUrl}`);
      }
      ...
      // Convertir a base64 para usar en <image>
      const base64Image = `data:image/png;base64,${pngBuffer.toString('base64')}`;
      console.log(`[FloppyRenderer] 🌐 LÓGICA EXTERNA: Trait ${traitId} cargado exitosamente`);
      return base64Image;
    } catch (error) {
      console.error(`[FloppyRenderer] 🌐 LÓGICA EXTERNA: Error cargando trait ${traitId}:`, error.message);
      return null;
    }
  }
  ...
  async #loadMetadata(tokenIdNum) {
    ...
    if (tokenIdNum === 262144) {
      ...
      // V1 legacy ids (30000-30300) viven en studio.json desde siempre.
      // V2 ids (30301+) sólo existen on-chain y se sintetizan al vuelo si
      // studio.json todavía no tiene entry (caso normal — el sync script
      // appendea entries en background pero el render no debe esperar).
      ...
      try {
        ...
        if (!traitData && isTShitV2(tokenIdNum)) {
          // V2 fallback: synthesize the same shape studio.json would have.
          // The on-chain TShitMintFacet is the source of truth — if the design
          // URI resolves we know the token exists; otherwise it's unminted.
          const uri = await resolveTShitUri(tokenIdNum);
          if (uri) {
            const ordinal = tokenIdNum - 30000 + 1; // human-friendly #N (continues V1 numbering)
            traitData = {
            ...
            console.log(`[FloppyRenderer] V2 metadata synthesised for ${tokenIdNum}`);
          }
        }
      } catch (error) {
  ...
  async #getTotalMinted(tokenId, tokenData) {
    ...
    try {
      ...
      const { traitsCore } = await Promise.race([
        getContracts(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('getContracts timeout after 8000ms')), 8000)),
      ]);
      const mintedAmount = await Promise.race([
        traitsCore.totalMintedPerAsset(tokenId),
        new Promise((_, reject) => setTimeout(() => reject(new Error('totalMintedPerAsset timeout after 5000ms')), 5000)),
      ]);
  ...
  #getRarityTagAndColor(maxSupply, tokenIdNum) {
    ...
    // No rarity tag for high supply items (airdrop backgrounds, etc.)
    if (maxSupply >= 1000) return { tag: null, bg: 'transparent' };
  ...
  async #generateStaticSvg(tokenId, tokenData, totalMinted, rarity) {
    ...
    const completeSvg = `
      <svg width="768" height="1024" xmlns="http://www.w3.org/2000/svg">
        ...
        ${rarity.tag ? `<rect x="84" y="120" width="160" height="60" fill="${rarity.bg}"/>
        ${textToSVGElement(rarity.tag, {
          x: 84 + 160 / 2,
          y: 120 + 60 / 2,
          fontSize: 32,
