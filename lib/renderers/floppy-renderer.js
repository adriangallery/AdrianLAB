...
export class FloppyRenderer {
  ...
  async generateSVG(tokenId, options = {}) {
    ...
    // Lógica especial para token 262144 (GIF directo)
    if (tokenIdNum === 262144) {
      console.log('[FloppyRenderer] 🧬 LÓGICA ESPECIAL: Token 262144 detectado');
      throw new Error('Token 262144 requiere manejo especial de GIF');
    }
  ...
  async generatePNG(tokenId, options = {}) {
    ...
    // Lógica especial para token 262144 (GIF directo)
    if (tokenIdNum === 262144) {
      console.log('[FloppyRenderer] 🧬 LÓGICA ESPECIAL: Token 262144 detectado, sirviendo GIF directamente');
      
      const gifPath = path.join(process.cwd(), 'public', 'labimages', `${tokenId}.gif`);
      console.log(`[FloppyRenderer] Ruta GIF: ${gifPath}`);
      
      if (fs.existsSync(gifPath)) {
        const gifBuffer = fs.readFileSync(gifPath);
        console.log(`[FloppyRenderer] GIF leído, tamaño: ${gifBuffer.length} bytes`);
        return gifBuffer;
      } else {
        throw new Error('GIF no encontrado para serum ADRIANGF');
      }
    }
  ...
  async #loadTraitFromLabimages(traitId) {
    try {
      ...
      console.log(`[FloppyRenderer] Cargando trait desde labimages: ${imageUrl}`);
