...
export class FloppyRenderer {
  ...
  async #loadAndDetectAnimation(svgFileName) {
    try {
      ...
      const isOgpunk = (numericId >= 100001 && numericId <= 101000) || (numericId >= 101001 && numericId <= 101002);
      const imageUrl = isOgpunk
        ? `${baseUrl}/labimages/ogpunks/${numericId}.svg`
        : `${baseUrl}/labimages/${svgFileName}`;
  ...
  async #loadTraitFromLabimages(traitId) {
    try {
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://adrianlab.vercel.app';
      const isOgpunk = (parseInt(traitId) >= 100001 && parseInt(traitId) <= 101000) || (parseInt(traitId) >= 101001 && parseInt(traitId) <= 101002);
      const imageUrl = isOgpunk
    ...
    const completeSvg = `
      <svg width="768" height="1024" xmlns="http://www.w3.org/2000/svg">
        ...
        ${linesToSVG([
          ...
          {
            text: `${(((tokenIdNum >= 100001 && tokenIdNum <= 101000) || (tokenIdNum >= 101001 && tokenIdNum <= 101002)) ? ('AdrianPunk #' + (tokenIdNum - 100000)) : ('FLOPPY: ' + (tokenData.floppy || 'OG')))}`,
            x: 84 + 10,  // Margen izquierdo de 10px
            y: 950,
            fontSize: 32,  // Aumentado de 24 a 32
            fill: '#333333',
