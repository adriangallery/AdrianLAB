...
export class AdrianZeroRenderer {
  ...
  async generatePNGForLambo(tokenId, options = {}) {
    ...
    // 1. Cargar AdrianZERO completo desde el endpoint original (reutilizar completamente)
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://adrianlab.vercel.app';
    const adrianUrl = `${baseUrl}/api/render/${tokenId}.png`;
    console.log(`[AdrianZeroRenderer] Cargando AdrianZERO completo desde: ${adrianUrl}`);
    
    const response = await fetch(adrianUrl);
    if (!response.ok) {
      throw new Error(`Error cargando AdrianZERO: ${response.status}`);
    }
    const adrianBuffer = await response.arrayBuffer();
    const adrianImage = await loadImage(Buffer.from(adrianBuffer));
    console.log('[AdrianZeroRenderer] AdrianZERO completo cargado correctamente con BACKGROUND');
