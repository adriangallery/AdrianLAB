...
import { getTokenDupInfo, getEffectiveGeneration } from '../duplicator-logic.js';
import { GEAR_BEHIND_BODY } from '../v2/shared/constants.js';
...
export class AdrianZeroRenderer {
  ...
  async #generatePNGInternalForLambo(tokenId) {
    ...
    try {
      ...
      if (dupInfo && dupInfo.duplicated) {
        console.log(`[AdrianZeroRenderer] 🔄 DUPLICATOR: Token ${cleanTokenId} está duplicado (dupNumber=${dupInfo.dupNumber})`);
      }
    } catch (error) {
      console.error(`[AdrianZeroRenderer] ⚠️ Error obteniendo dupInfo para token ${cleanTokenId}:`, error.message);
    }
    ...
    if (dupInfo && dupInfo.duplicated) {
      console.log(`[AdrianZeroRenderer] 🔄 DUPLICATOR: Usando sourceId ${serumSourceTokenId} para consulta de serum`);
    }
    ...
    for (const category of traitOrder) {
      if (equippedTraits[category]) {
        // GEAR «colgado en la pared» (Freed Soul): estas vías no pintan fondo, así que no hay pared y no va encima del cuerpo
        if (category === 'GEAR' && GEAR_BEHIND_BODY.has(parseInt(equippedTraits['GEAR']))) continue;
  ...
  async #generatePNGWithoutBackground(tokenId) {
    ...
    if (tagInfo.tag === 'SubZERO') {
      console.log(`[AdrianZeroRenderer] 🏷️ Token ${cleanTokenId} tiene tag SubZERO - Aplicando lógica especial`);
      ...
      console.log(`[AdrianZeroRenderer] 🏷️ SubZERO: EYES filtrado, SKINTRAIT 1125 forzado con prioridad absoluta`);
    }
    
    // ===== LÓGICA ESPECIAL SAMURAIZERO =====
    if (tagInfo.tag === 'SamuraiZERO') {
      console.log(`[AdrianZeroRenderer] 🥷 Token ${cleanTokenId} tiene tag SamuraiZERO - Aplicando lógica especial`);
      ...
      if (samuraiIndex !== null && samuraiIndex >= 0 && samuraiIndex < 600) {
        ...
        console.log(`[AdrianZeroRenderer] 🥷 SamuraiZERO token ${cleanTokenId} tiene índice ${samuraiIndex}, usando imagen ${imageIndex}.svg como TOP`);
        
        // Forzar trait TOP con la imagen de SamuraiZERO
        equippedTraits['TOP'] = imageIndex.toString();
        
        console.log(`[AdrianZeroRenderer] 🥷 SamuraiZERO: TOP ${imageIndex} forzado, se renderizará sobre todo lo demás`);
      } else {
        console.error(`[AdrianZeroRenderer] 🥷 SamuraiZERO token ${cleanTokenId} tiene índice inválido: ${samuraiIndex}`);
      }
    }
    ...
    let dupInfo2 = null;
    try {
      dupInfo2 = await getTokenDupInfo(duplicatorModule, cleanTokenId);
      if (dupInfo2 && dupInfo2.duplicated) {
        console.log(`[AdrianZeroRenderer] 🔄 DUPLICATOR: Token ${cleanTokenId} está duplicado (dupNumber=${dupInfo2.dupNumber})`);
      }
    } catch (error) {
      console.error(`[AdrianZeroRenderer] ⚠️ Error obteniendo dupInfo para token ${cleanTokenId}:`, error.message);
    }
    ...
    // Determinar el tokenId fuente para consultas de serum (duplicados heredan del parent)
    const serumSourceTokenId2 = (dupInfo2 && dupInfo2.duplicated && dupInfo2.sourceId) ? dupInfo2.sourceId : cleanTokenId;
    if (dupInfo2 && dupInfo2.duplicated) {
      console.log(`[AdrianZeroRenderer] 🔄 DUPLICATOR: Usando sourceId ${serumSourceTokenId2} para consulta de serum`);
    }
    ...
    try {
      console.log('[AdrianZeroRenderer] Verificando si hay serum aplicado...');
      const serumHistory = await serumModule.getTokenSerumHistory(serumSourceTokenId2);
    ...
    if (skintraitPath) {
      ...
      // Skin normal - usar generación efectiva
      const gen = getEffectiveGeneration(dupInfo2, generation);
  ...
  async #generatePNGInternal(tokenId) {
    ...
    let dupInfo3 = null;
    try {
      dupInfo3 = await getTokenDupInfo(duplicatorModule, cleanTokenId);
      if (dupInfo3 && dupInfo3.duplicated) {
        console.log(`[AdrianZeroRenderer] 🔄 DUPLICATOR: Token ${cleanTokenId} está duplicado (dupNumber=${dupInfo3.dupNumber})`);
      }
    } catch (error) {
      console.error(`[AdrianZeroRenderer] ⚠️ Error obteniendo dupInfo para token ${cleanTokenId}:`, error.message);
    }
    ...
    // Determinar el tokenId fuente para consultas de serum (duplicados heredan del parent)
    const serumSourceTokenId3 = (dupInfo3 && dupInfo3.duplicated && dupInfo3.sourceId) ? dupInfo3.sourceId : cleanTokenId;
    if (dupInfo3 && dupInfo3.duplicated) {
      console.log(`[AdrianZeroRenderer] 🔄 DUPLICATOR: Usando sourceId ${serumSourceTokenId3} para consulta de serum`);
    }

    // Determinar la imagen base según generación y skin
    // Si el token está duplicado, usar dupNumber como generación efectiva
    const gen = getEffectiveGeneration(dupInfo3, generation);
    let baseImagePath;
    ...
    try {
      console.log('[AdrianZeroRenderer] Verificando si hay serum aplicado...');
      const serumHistory = await serumModule.getTokenSerumHistory(serumSourceTokenId3);
    ...
    for (const category of topOrder) {
      if (equippedTraits[category]) {
        ...
        if (traitId >= 30000 && traitId <= 35000) {
          ...
          // LÓGICA ESPECIAL: SamuraiZERO traits (500-1099) se cargan desde samuraizero/ (con fallback a GitHub)
          let traitImage;
          if (traitId >= 500 && traitId <= 1099) {
            ...
            console.log(`[AdrianZeroRenderer] 🥷 Cargando SamuraiZERO trait desde (con fallback GitHub): ${assetPath}`);
            ...
            try {
              ...
              console.error(`[AdrianZeroRenderer] 🥷 Error al cargar SamuraiZERO trait ${traitId}:`, error.message);
            }
          } else {
          ...
          if (traitImage) {
            ctx.drawImage(traitImage, 0, 0, 1000, 1000);
            if (traitId >= 500 && traitId <= 1099) {
              console.log(`[AdrianZeroRenderer] 🥷 PASO 4 - TOP trait SamuraiZERO ${category} (${traitId}) renderizado desde samuraizero/ correctamente`);
            } else {
              console.log(`[AdrianZeroRenderer] PASO 4 - TOP trait ${category} (${traitId}) renderizado desde labimages correctamente`);
            }
          } else {
