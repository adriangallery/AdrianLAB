...
export class AdrianZeroRenderer {
  ...
  async generatePNGForLambo(tokenId, options = {}) {
    ...
    // 1. Generar AdrianZERO completo con fondo transparente (para lambos)
    console.log('[AdrianZeroRenderer] Generando AdrianZERO completo con fondo transparente para lambo...');
    const adrianBuffer = await this.#generatePNGInternalForLambo(tokenId);
  ...
  async #generatePNGInternalForLambo(tokenId) {
    const cleanTokenId = tokenId.toString().replace('.png', '');
    console.log(`[AdrianZeroRenderer] Iniciando renderizado para lambo (fondo transparente) para token ${cleanTokenId}`);
    ...
    console.log('[AdrianZeroRenderer] Traits equipados:', equippedTraits);

    // Crear canvas con fondo TRANSPARENTE (para lambos)
    const canvas = createCanvas(1000, 1000);
    const ctx = canvas.getContext('2d');
    ...
    const loadAndRenderSvg = async (svgPath, x = 0, y = 0, width = 1000, height = 1000) => {
      try {
        const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://adrianlab.vercel.app';
        const imageUrl = `${baseUrl}/traits/${svgPath}`;
        console.log(`[AdrianZeroRenderer] Cargando SVG: ${imageUrl}`);
        ...
        console.log(`[AdrianZeroRenderer] SVG renderizado: ${svgPath}`);
        return true;
      } catch (error) {
    ...
    // Renderizar BACKGROUND si existe (pero NO llenar el canvas)
    if (equippedTraits['BACKGROUND']) {
      console.log(`[AdrianZeroRenderer] Renderizando BACKGROUND ${equippedTraits['BACKGROUND']} (transparente para lambo)`);
      const bgPath = `BACKGROUND/${equippedTraits['BACKGROUND']}.svg`;
      await loadAndRenderSvg(bgPath);
    }

    // Renderizar SKIN
    if (equippedTraits['SKIN']) {
      console.log(`[AdrianZeroRenderer] Renderizando SKIN ${equippedTraits['SKIN']}`);
      const skinPath = `SKIN/${equippedTraits['SKIN']}.svg`;
      await loadAndRenderSvg(skinPath);
    }

    // Renderizar EYES
    if (equippedTraits['EYES']) {
      console.log(`[AdrianZeroRenderer] Renderizando EYES ${equippedTraits['EYES']}`);
      const eyesPath = `EYES/${equippedTraits['EYES']}.svg`;
      await loadAndRenderSvg(eyesPath);
    }

    // Renderizar MOUTH
    if (equippedTraits['MOUTH']) {
      console.log(`[AdrianZeroRenderer] Renderizando MOUTH ${equippedTraits['MOUTH']}`);
      const mouthPath = `MOUTH/${equippedTraits['MOUTH']}.svg`;
      await loadAndRenderSvg(mouthPath);
    }

    // Renderizar NOSE
    if (equippedTraits['NOSE']) {
      console.log(`[AdrianZeroRenderer] Renderizando NOSE ${equippedTraits['NOSE']}`);
      const nosePath = `NOSE/${equippedTraits['NOSE']}.svg`;
      await loadAndRenderSvg(nosePath);
    }

    // Renderizar EYEBROWS
    if (equippedTraits['EYEBROWS']) {
      console.log(`[AdrianZeroRenderer] Renderizando EYEBROWS ${equippedTraits['EYEBROWS']}`);
      const eyebrowsPath = `EYEBROWS/${equippedTraits['EYEBROWS']}.svg`;
      await loadAndRenderSvg(eyebrowsPath);
    }

    // Renderizar HAIR
    if (equippedTraits['HAIR']) {
      console.log(`[AdrianZeroRenderer] Renderizando HAIR ${equippedTraits['HAIR']}`);
      const hairPath = `HAIR/${equippedTraits['HAIR']}.svg`;
      await loadAndRenderSvg(hairPath);
    }

    // Renderizar FACIAL_HAIR
    if (equippedTraits['FACIAL_HAIR']) {
      console.log(`[AdrianZeroRenderer] Renderizando FACIAL_HAIR ${equippedTraits['FACIAL_HAIR']}`);
      const facialHairPath = `FACIAL_HAIR/${equippedTraits['FACIAL_HAIR']}.svg`;
      await loadAndRenderSvg(facialHairPath);
    }

    // Renderizar EARRINGS
    if (equippedTraits['EARRINGS']) {
      console.log(`[AdrianZeroRenderer] Renderizando EARRINGS ${equippedTraits['EARRINGS']}`);
      const earringsPath = `EARRINGS/${equippedTraits['EARRINGS']}.svg`;
      await loadAndRenderSvg(earringsPath);
    }

    // Renderizar NECKLACE
    if (equippedTraits['NECKLACE']) {
      console.log(`[AdrianZeroRenderer] Renderizando NECKLACE ${equippedTraits['NECKLACE']}`);
      const necklacePath = `NECKLACE/${equippedTraits['NECKLACE']}.svg`;
      await loadAndRenderSvg(necklacePath);
    }

    // Renderizar CLOTHING
    if (equippedTraits['CLOTHING']) {
      console.log(`[AdrianZeroRenderer] Renderizando CLOTHING ${equippedTraits['CLOTHING']}`);
      const clothingPath = `CLOTHING/${equippedTraits['CLOTHING']}.svg`;
      await loadAndRenderSvg(clothingPath);
    }

    // Renderizar HAT
    if (equippedTraits['HAT']) {
      console.log(`[AdrianZeroRenderer] Renderizando HAT ${equippedTraits['HAT']}`);
      const hatPath = `HAT/${equippedTraits['HAT']}.svg`;
      await loadAndRenderSvg(hatPath);
    }

    // Renderizar GLASSES
    if (equippedTraits['GLASSES']) {
      console.log(`[AdrianZeroRenderer] Renderizando GLASSES ${equippedTraits['GLASSES']}`);
      const glassesPath = `GLASSES/${equippedTraits['GLASSES']}.svg`;
      await loadAndRenderSvg(glassesPath);
    }

    // Renderizar MASK
    if (equippedTraits['MASK']) {
      console.log(`[AdrianZeroRenderer] Renderizando MASK ${equippedTraits['MASK']}`);
      const maskPath = `MASK/${equippedTraits['MASK']}.svg`;
      await loadAndRenderSvg(maskPath);
    }

    // Renderizar TOP (última capa)
    if (equippedTraits['TOP']) {
      console.log(`[AdrianZeroRenderer] Renderizando TOP ${equippedTraits['TOP']}`);
      const topPath = `TOP/${equippedTraits['TOP']}.svg`;
      await loadAndRenderSvg(topPath);
    }

    // Verificar si es Patient Zero
    if (generation.toString() === '0') {
      console.log('[AdrianZeroRenderer] Token es Patient Zero, renderizando elementos especiales...');
      
      // Renderizar elementos de Patient Zero si es necesario
      // (mantener la misma lógica que el render normal)
    }

    // Verificar si tiene serums
    if (mutationLevel.toString() !== '0') {
      console.log(`[AdrianZeroRenderer] Token tiene serums (nivel ${mutationLevel}), renderizando efectos...`);
      
      // Renderizar efectos de serums si es necesario
      // (mantener la misma lógica que el render normal)
    }

    console.log(`[AdrianZeroRenderer] Renderizado completado para token ${cleanTokenId} (fondo transparente para lambo)`);
    return canvas.toBuffer('image/png');
  }
