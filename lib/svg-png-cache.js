...
/**
 * Obtiene el TTL específico para conversiones SVG→PNG de SamuraiZERO
 * @param {string} svgContent - Contenido del SVG
 * @returns {number} - TTL en millisegundos
 */
export function getSamuraiSvgPngTTL(svgContent) {
  // Detectar si es SamuraiZERO por contenido
  if (svgContent.includes('samurai') || svgContent.includes('SamuraiZERO') || svgContent.includes('samuraizero')) {
...
export function setCachedSvgPng(svgContent, pngBuffer) {
  const hash = createHash('md5').update(svgContent).digest('hex');
  const ttl = getSamuraiSvgPngTTL(svgContent);
  ...
  console.log(`[svg-png-cache] 💾 CACHE MISS: SVG→PNG (${pngBuffer.length} bytes, TTL: ${Math.round(ttl/3600000)}h)`);
}
