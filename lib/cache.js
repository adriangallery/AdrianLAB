...
export function getAdrianZeroRenderTTL(tokenId) {
  ...
  if (tokenIdNum >= 500 && tokenIdNum <= 1099) {
    return 2592000000; // 30 days - SamuraiZERO (static images, never change)
  } else if (tokenIdNum >= 1 && tokenIdNum <= 9999) {
    return 604800000; // 7 days - Normal tokens (traits change rarely)
  } else if (tokenIdNum >= 30000 && tokenIdNum <= 35000) {
    return 2592000000; // 30 days - T-shirts personalizados (static)
  } else if (tokenIdNum === 262144) {
    return 2592000000; // 30 days - Serum (static)
  } else {
    return 86400000; // 24h - Fallback
  }
}
 ...
 */
function getAdrianZeroRenderCacheKey(tokenId, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false, isBanana = false, bounceConfig = null, messageText = null) {
  const effects = [];
  ...
  if (bounceConfig && bounceConfig.enabled) {
    const bounceStr = `${bounceConfig.direction}:${bounceConfig.distance}:${bounceConfig.bounces}:${bounceConfig.frames}:${bounceConfig.delay}`;
    let hash = 0;
    for (let i = 0; i < bounceStr.length; i++) {
      const char = bounceStr.charCodeAt(i);
      hash = ((hash << 5) - hash) + char;
      hash = hash & hash;
    }
    const bounceHash = Math.abs(hash).toString(36);
    effects.push(`bounce_${bounceHash}`);
  }
  if (messageText) {
    // Crear hash del texto del mensaje para evitar claves muy largas
    let hash = 0;
    ...
    const messageHash = Math.abs(hash).toString(36);
    effects.push(`messages_${messageHash}`);
  }
 ...
 * @returns {string} - Clave de caché
 */
function getAdrianZeroCustomRenderCacheKey(tokenId, finalTraits, isCloseup = false, bounceConfig = null) {
  ...
  const parts = [`adrianzero_custom_${tokenId}_${traitsHash}`];
  if (isCloseup) parts.push('closeup');
  if (bounceConfig && bounceConfig.enabled) {
    const bounceStr = `${bounceConfig.direction}:${bounceConfig.distance}:${bounceConfig.bounces}:${bounceConfig.frames}:${bounceConfig.delay}`;
    let bounceHash = 0;
    for (let i = 0; i < bounceStr.length; i++) {
      const char = bounceStr.charCodeAt(i);
      bounceHash = ((bounceHash << 5) - bounceHash) + char;
      bounceHash = bounceHash & bounceHash;
    }
    parts.push(`bounce_${Math.abs(bounceHash).toString(36)}`);
  }
 ...
 * @returns {Buffer|null} - Imagen cacheada o null
 */
export function getCachedAdrianZeroRender(tokenId, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false, isBanana = false, messageText = null) {
  const key = getAdrianZeroRenderCacheKey(tokenId, isShadow, isGlow, isBn, isUv, isBlackout, isBanana, null, messageText);
 ...
 * @param {boolean} isBlackout - Si blackout está activo
 */
export function setCachedAdrianZeroRender(tokenId, imageBuffer, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false, isBanana = false, messageText = null) {
  const ttl = getAdrianZeroRenderTTL(tokenId);
  const key = getAdrianZeroRenderCacheKey(tokenId, isShadow, isGlow, isBn, isUv, isBlackout, isBanana, null, messageText);
  setCachedResult(key, imageBuffer, ttl);
  
  const effectsStr = [isShadow && 'shadow', isGlow && 'glow', isBn && 'bn', isUv && 'uv', isBlackout && 'blackout', isBanana && 'banana', messageText && 'messages'].filter(Boolean).join('+');
...
/**
 * Genera una clave de caché única para GIF de AdrianZero basada en tokenId, finalTraits y bounceConfig
 * @param {string|number} tokenId - Token ID
 * @param {Object} finalTraits - Objeto con los traits finales { CATEGORY: traitId, ... } (opcional)
 * @param {Object} bounceConfig - Configuración de bounce { enabled, direction, distance, bounces, frames, delay } (opcional)
 * @returns {string} - Clave de caché
 */
function getAdrianZeroGifCacheKey(tokenId, finalTraits = null, bounceConfig = null) {
  const parts = [`adrianzero_gif_${tokenId}`];
  
  // Añadir hash de traits si existen
  if (finalTraits && Object.keys(finalTraits).length > 0) {
    ...
    for (let i = 0; i < traitsEntries.length; i++) {
      const char = traitsEntries.charCodeAt(i);
      hash = ((hash << 5) - hash) + char;
      hash = hash & hash;
    }
    const traitsHash = Math.abs(hash).toString(36);
    parts.push(traitsHash);
  }
  
  // Añadir hash de bounce si está activo (incluye params nuevos: squash, stretch, anchorY, frameMs)
  if (bounceConfig && bounceConfig.enabled) {
    const bounceStr = [
      ...
      bounceConfig.frames,
      bounceConfig.delay,
      bounceConfig.squash || 0,
      bounceConfig.stretch || 0,
      bounceConfig.anchorY || 0.5,
      bounceConfig.frameMs || 0,
    ].join(':');
    let hash = 0;
    for (let i = 0; i < bounceStr.length; i++) {
      const char = bounceStr.charCodeAt(i);
 ...
 * @param {string|number} tokenId - Token ID
 * @param {Object} finalTraits - Objeto con los traits finales { CATEGORY: traitId, ... } (opcional)
 * @param {Object} bounceConfig - Configuración de bounce (opcional)
 * @returns {Buffer|null} - GIF cacheado o null
 */
export function getCachedAdrianZeroGif(tokenId, finalTraits = null, bounceConfig = null) {
  const key = getAdrianZeroGifCacheKey(tokenId, finalTraits, bounceConfig);
  return getCachedResult(key);
}
 ...
 * @param {Object} bounceConfig - Configuración de bounce (opcional)
 */
export function setCachedAdrianZeroGif(tokenId, gifBuffer, finalTraits = null, bounceConfig = null) {
  const ttl = getAdrianZeroRenderTTL(tokenId); // Mismo TTL que PNGs
  const key = getAdrianZeroGifCacheKey(tokenId, finalTraits, bounceConfig);
  setCachedResult(key, gifBuffer, ttl);
  const traitsInfo = finalTraits ? ` (con ${Object.keys(finalTraits).length} traits personalizados)` : '';
  const bounceInfo = bounceConfig && bounceConfig.enabled ? ' + bounce' : '';
  console.log(`[cache] Cached AdrianZero GIF: token ${tokenId}${traitsInfo}${bounceInfo} for ${Math.floor(ttl/3600000)}h`);
}
