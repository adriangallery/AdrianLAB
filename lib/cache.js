 ...
 */
function getAdrianZeroRenderCacheKey(tokenId, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false) {
  const effects = [];
 ...
 * @returns {Buffer|null} - Imagen cacheada o null
 */
export function getCachedAdrianZeroRender(tokenId, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false) {
  const key = getAdrianZeroRenderCacheKey(tokenId, isShadow, isGlow, isBn, isUv, isBlackout);
 ...
 * @param {boolean} isBlackout - Si blackout está activo
 */
export function setCachedAdrianZeroRender(tokenId, imageBuffer, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false) {
  const ttl = getAdrianZeroRenderTTL(tokenId);
  const key = getAdrianZeroRenderCacheKey(tokenId, isShadow, isGlow, isBn, isUv, isBlackout);
  setCachedResult(key, imageBuffer, ttl);
  
  const effectsStr = [isShadow && 'shadow', isGlow && 'glow', isBn && 'bn', isUv && 'uv', isBlackout && 'blackout'].filter(Boolean).join('+');
 ...
 * @returns {string} - Clave de caché
 */
function getAdrianZeroCloseupCacheKey(tokenId, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false) {
  const effects = [];
  if (isShadow) effects.push('shadow');
  if (isGlow) effects.push('glow');
  if (isBn) effects.push('bn');
 ...
 */
export function getCachedAdrianZeroCloseup(tokenId, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false) {
  const key = getAdrianZeroCloseupCacheKey(tokenId, isShadow, isGlow, isBn, isUv, isBlackout);
  return getCachedResult(key);
}
 ...
 * @param {boolean} isBlackout - Si blackout está activo
 */
export function setCachedAdrianZeroCloseup(tokenId, imageBuffer, isShadow = false, isGlow = false, isBn = false, isUv = false, isBlackout = false) {
  const ttl = getAdrianZeroRenderTTL(tokenId);
  const key = getAdrianZeroCloseupCacheKey(tokenId, isShadow, isGlow, isBn, isUv, isBlackout);
  setCachedResult(key, imageBuffer, ttl);
