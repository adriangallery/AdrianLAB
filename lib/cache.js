 ...
 */
export function invalidateAdrianZeroRender(tokenId) {
  const cacheKey = `adrianzero_render_${tokenId}`;
  const existed = memoryCache.has(cacheKey);
 ...
 */
export function invalidateFloppyRender(tokenId) {
  const cacheKey = `floppy_render_${tokenId}`;
  const existed = memoryCache.has(cacheKey);
  memoryCache.delete(cacheKey);
 ...
 */
export function invalidateFloppyMetadata(tokenId) {
  const cacheKey = `floppy_metadata_${tokenId}`;
  const existed = memoryCache.has(cacheKey);
  memoryCache.delete(cacheKey);
