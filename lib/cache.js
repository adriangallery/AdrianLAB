 ...
 */
export function getCachedAdrianZeroCloseup(tokenId) {
  return getCachedResult(`adrianzero_closeup_${tokenId}`);
}
 ...
 */
export function setCachedAdrianZeroCloseup(tokenId, imageBuffer) {
  const ttl = getAdrianZeroRenderTTL(tokenId);
  setCachedResult(`adrianzero_closeup_${tokenId}`, imageBuffer, ttl);
}

/**
 * Invalida caché de AdrianZero closeup por token específico
 * @param {string|number} tokenId - Token ID
 * @returns {boolean} - true si se invalidó, false si no existía
 */
export function invalidateAdrianZeroCloseup(tokenId) {
  const key = `adrianzero_closeup_${tokenId}`;
  if (memoryCache.has(key)) {
    memoryCache.delete(key);
    console.log(`[cache] Invalidated AdrianZero closeup for token ${tokenId}`);
    return true;
  }
  return false;
}
