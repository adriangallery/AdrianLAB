...
/**
 * Genera una clave de caché única para GIF de AdrianZero basada en tokenId y finalTraits
 * @param {string|number} tokenId - Token ID
 * @param {Object} finalTraits - Objeto con los traits finales { CATEGORY: traitId, ... } (opcional)
 * @returns {string} - Clave de caché
 */
function getAdrianZeroGifCacheKey(tokenId, finalTraits = null) {
  if (!finalTraits || Object.keys(finalTraits).length === 0) {
    // Sin traits personalizados, usar solo tokenId (comportamiento original)
    return `adrianzero_gif_${tokenId}`;
  }
  ...
  return `adrianzero_gif_${tokenId}_${traitsHash}`;
}
 ...
 */
export function getCachedAdrianZeroGif(tokenId, finalTraits = null) {
  const key = getAdrianZeroGifCacheKey(tokenId, finalTraits);
  return getCachedResult(key);
}
 ...
 * @param {Object} finalTraits - Objeto con los traits finales { CATEGORY: traitId, ... } (opcional)
 */
export function setCachedAdrianZeroGif(tokenId, gifBuffer, finalTraits = null) {
  ...
  console.log(`[cache] Cached AdrianZero GIF: token ${tokenId}${traitsInfo} for ${Math.floor(ttl/3600000)}h`);
}
